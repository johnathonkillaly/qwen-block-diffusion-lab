...
0. **There are two different "Act IV"s.** `Act IV-N` is the structured-noise alphabet
   ...
   one. It has four phases: `U` (Uno reproduction), `U2` (transactional recurrent
   verification), `U3` (acceptance scaling) and `U4` (horizon scaling). They share no
   code with Act IV-N. Don't cross the streams.
...
2. **Pre-registered criteria are frozen.** `docs/ACT3_CRITERIA.md`,
   ...
   and `docs/act4u4_preregistered_criteria.md`
