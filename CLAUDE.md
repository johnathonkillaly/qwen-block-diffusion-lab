...
0. **There are two different "Act IV"s.** `Act IV-N` is the structured-noise alphabet
   ...
   track (`src/qdif/uno/`, `scripts/uno.py`, `docs/act4u_*.md`), which is the active
   one. They share no code. Don't cross the streams.
...
2. **Pre-registered criteria are frozen.** `docs/ACT3_CRITERIA.md`,
   `docs/ACT4_CRITERIA.md` and `docs/act4u_preregistered_criteria.md` must not be
   edited after results exist. Amendments get appended with a date and a reason.
